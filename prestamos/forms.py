...
class PrestamoForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'fecha_prestamo': forms.DateInput(format='%Y-%m-%d', attrs={'type': 'date', 'id': 'id_fecha_prestamo', 'class': 'form-control'}),
'fecha_vencimiento': forms.DateInput(format='%Y-%m-%d', attrs={'type': 'date', 'id': 'id_fecha_vencimiento', 'class': 'form-control'}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:  # Para préstamos existentes (edición)
            self.initial['fecha_prestamo'] = self.instance.fecha_prestamo
            self.initial['fecha_vencimiento'] = self.instance.fecha_vencimiento
            if self.instance.cliente:
                self.initial['cliente_busqueda'] = f"{self.instance.cliente.nombre} - {self.instance.cliente.email} - {self.instance.cliente.telefono}"
        else:  # Para nuevos préstamos
            fecha_actual = date.today()
            fecha_vencimiento = fecha_actual + timedelta(days=30)
            self.initial['fecha_prestamo'] = fecha_actual
            self.initial['fecha_vencimiento'] = fecha_vencimiento
